...
// ─── Helpers (Duplicated from certificateUtils for standalone nature as requested) ─────────────
...
const getBase64ImageFromUrl = async (url: string): Promise<string | null> => {
  try {
    ...
    console.error("Scout Certificate: image load error", e);
    return null;
  }
};
...
const getFontBase64 = async (path: string): Promise<string | null> => {
  try {
    ...
    console.error("Scout Certificate: font load error", e);
    return null;
  }
};
...
async function drawScoutCertificate(
  doc: jsPDF,
  name: string,
  schoolName: string,
  ...
  // Positioning logic matches the GjamD logic for now, but background is GjamS.jpeg
  const PT = 2.8346;
  const x = 382.8079 / PT;
...
/**
 * Generates and downloads a Scout Team Participation Certificate PDF.
 */
export async function generateScoutCertificate(
  records: any[],
  filename: string
) {
  ...
  const [fonts, bgBase64] = await Promise.all([
    ...
    getBase64ImageFromUrl("/certificate/GjamS.jpeg"), // Requested Scout Background
  ]);
  ...
  for (let i = 0; i < records.length; i++) {
    ...
    await drawScoutCertificate(doc, name, schoolName, bgBase64, fonts);
  }
