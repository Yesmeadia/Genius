    ...
    <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mb-10">
      ...
      <Card className="lg:col-span-8 border-none shadow-sm shadow-slate-200/50 rounded-[32px] bg-white p-6">
        <CardHeader className="flex flex-row items-center justify-between pb-6">
          <div>
            <CardTitle className="text-xl font-black text-slate-900">Registration Trend</CardTitle>
            <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Daily registration flow volume</p>
          </div>
          <div className="flex bg-slate-100 p-1 rounded-xl">
             <button className="px-4 py-1.5 text-[10px] font-black uppercase text-slate-400">Monthly</button>
             <button className="px-4 py-1.5 text-[10px] font-black uppercase bg-blue-600 text-white rounded-lg shadow-sm">Daily</button>
          </div>
        </CardHeader>
        <CardContent className="h-[350px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={zoneData}>
              ...
              <XAxis dataKey="zone" fontSize={10} axisLine={false} tickLine={false} tick={{fill: '#94a3b8', fontWeight: 700}} />
      ...
      <Card className="lg:col-span-4 border-none shadow-sm shadow-slate-200/50 rounded-[32px] bg-white p-6 relative overflow-hidden">
        ...
        <CardHeader className="pb-6 relative">
          <CardTitle className="text-xl font-black text-slate-900">Student Sources</CardTitle>
          <p className="text-xs font-bold text-slate-400 uppercase tracking-widest mt-1">Submissions by gender</p>
        </CardHeader>
        <CardContent>
              ...
              return (
                <div key={i} className="flex items-center justify-between p-3 rounded-2xl bg-slate-50/50 hover:bg-slate-50 transition-colors">
                  ...
                  <div className="text-right">
                    <div className="text-[14px] font-black text-slate-900">{item.value}</div>
      ...
      <Card className="lg:col-span-6 border-none shadow-sm shadow-slate-200/50 rounded-[32px] bg-white p-6">
        <CardHeader className="flex flex-row items-center justify-between pb-6">
            <CardTitle className="text-lg font-black text-slate-900">Guardian Relation</CardTitle>
        ...
        <CardContent className="h-[250px]">
            <ResponsiveContainer width="100%" height="100%">
                <BarChart data={relationData} layout="vertical">
                    ...
                    <YAxis dataKey="relation" type="category" fontSize={10} axisLine={false} tickLine={false} tick={{fill: '#475569', fontWeight: 800}} />
                    <Tooltip cursor={{fill: 'rgba(59, 130, 246, 0.05)'}} />
      ...
      <Card className="lg:col-span-6 border-none shadow-sm shadow-slate-200/50 rounded-[32px] bg-white p-6">
        <CardHeader className="flex flex-row items-center justify-between pb-6">
            <CardTitle className="text-lg font-black text-slate-900">Accompaniment Status</CardTitle>
            <Activity size={20} className="text-slate-200" />
        </CardHeader>
        <CardContent className="h-[250px]">
          <ResponsiveContainer width="100%" height="100%">
            <PieChart>
              <Pie data={accompanimentData} cx="50%" cy="50%" innerRadius={50} outerRadius={70} dataKey="value" stroke="none">
