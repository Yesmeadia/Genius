...
import {
  ...
  MapPin, Download, Trash2, CheckCircle2,
  MoreHorizontal, Camera, Briefcase, Building2
} from "lucide-react";
...
export default function MediaProfilePage() {
  ...
  const [registration, setRegistration] = useState<MediaRegistration | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchMedia = async () => {
      try {
        const docRef = doc(db, "media_registrations", id as string);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setRegistration({ id: docSnap.id, ...docSnap.data() } as MediaRegistration);
        } else {
  ...
  const updateField = (field: keyof MediaEditForm, value: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (photoFile) {
        const storageRef = ref(storage, `photos/media_${Date.now()}_${photoFile.name}`);
      ...
      const updateData = {
        ...
        agency: editForm.agency.toUpperCase(),
      ...
      await updateDoc(doc(db, "media_registrations", id as string), updateData);
  ...
  const handleDelete = async () => {
    ...
    const confirmDelete = window.confirm(`Are you sure you want to delete the record for ${registration.name}? This will be moved to the Recycle Bin.`);
            ...
            {editMode ? (
              ...
              !showActions ? (
                ...
                <div className="flex items-center gap-3 animate-in slide-in-from-right-4 duration-300">
                  ...
                  <Button
                    onClick={() => generateBatchAccessPasses([registration as any], `AccessPass_${registration.name.replace(/\s+/g, '_')}`, 'media')}
                    className="h-10 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white font-normal uppercase text-[10px] tracking-widest shadow-lg shadow-indigo-100"
                  >
                ...
                {editMode ? (
                  <>
                    <div className="space-y-2">
                      <Label className="text-[11px] font-normal text-slate-400 uppercase tracking-[0.2em]">Agency / Media House</Label>
                      <Input
                        value={editForm?.agency || ""}
                        onChange={e => updateField("agency", e.target.value.toUpperCase())}
                        className="uppercase font-normal text-slate-900 border-slate-100 bg-slate-50"
                  ...
                  <>
                    <div className="space-y-8">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <div className="text-[11px] font-normal text-slate-300 uppercase tracking-[0.2em] mb-1">Agency / Media House</div>
                          <div className="text-base font-normal text-slate-800 leading-snug">{registration.agency || "N/A"}</div>
                          <div className="text-[11px] text-slate-400 mt-1 uppercase tracking-tight">{registration.designation || ""}</div>
                        </div>
                      </div>
                    ...
                    <div className="space-y-8">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-base font-normal text-slate-800">{registration.whatsappNumber || "N/A"}</div>
                        </div>
                      </div>
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-base font-normal text-slate-800 capitalize">{registration.gender?.toLowerCase() || "N/A"}</div>
                        </div>
                      </div>
                    </div>
                  </>
