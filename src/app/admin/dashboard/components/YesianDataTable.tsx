...
import { Search, Download, RotateCcw, User, Phone, MapPin, ShieldCheck } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
...
interface YesianDataTableProps {
  data: YesianRegistration[];
  searchTerm: string;
  setSearchTerm: (term: string) => void;
  itemsPerPage: number;
}

export function YesianDataTable({
  data,
  searchTerm,
  setSearchTerm,
  itemsPerPage,
}: YesianDataTableProps) {
          ...
          <Button 
            onClick={() => {
              import("@/lib/exportUtils").then(m => {
                m.generateYesianExportPDF(data, "Yesian Network Registry", "yesian_network_data");
              });
            }}
                ...
                >
                  <TableCell className="py-4 pl-6">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-8 w-8 border border-slate-100 shadow-sm overflow-hidden">
                        {reg.photoUrl ? (
                  ...
                  <TableCell className="py-4">
                    ...
                    <div className="text-[10px] text-slate-400 uppercase tracking-tight flex items-center gap-1">
                      <MapPin size={9} />
                      {reg.zone}
                    </div>
                  </TableCell>
