...
export function AnalyticsSummary({ stats }: AnalyticsSummaryProps) {
  const statCards = [
    ...
    { 
      ...
      trend: `${stats.uniqueSchools} TOTAL SCHOOLS`,
    ...
    { 
      ...
      trend: `${stats.males} MALE / ${stats.females} FEMALE`,
    ...
    { 
      ...
      trend: `${stats.uniqueZones} TOTAL ZONES`,
        ...
        <Card key={i} className={`border-none shadow-sm shadow-slate-200/50 rounded-[32px] relative overflow-hidden bg-white border-l-4 ${item.borderColor}`}>
          <CardContent className="p-8">
            <div className="flex justify-between items-start mb-6">
              <div className="text-[11px] font-normal text-slate-400 uppercase tracking-widest">{item.label}</div>
              <div className={`w-12 h-12 ${item.color} rounded-2xl flex items-center justify-center shadow-sm`}>
                {item.icon}
              </div>
            </div>
            
            <div className="text-4xl font-normal text-slate-900 mb-6">{item.value}</div>
            
            <div className="flex items-center gap-2">
                <span className={`text-[9px] font-normal px-2.5 py-1 rounded-full uppercase tracking-wider ${item.trendColor}`}>
                    {item.trend}
                </span>
                <span className="text-[9px] font-normal text-slate-400 uppercase tracking-wider">Registration flow</span>
            </div>
          </CardContent>
        </Card>
      ))}
