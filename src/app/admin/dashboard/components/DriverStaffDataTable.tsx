...
import { Search, Download, RotateCcw, User, Phone, MapPin, Truck, ShieldCheck } from "lucide-react";
import {
  Table,
  TableBody,
  TableCell,
      ...
      <CardHeader className="bg-white p-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 border-b border-slate-50">
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
          <div className="relative flex-grow sm:w-64 max-w-full">
            ...
            />
            <Input
              placeholder="Search Drivers & Staff..."
              value={searchTerm}
                ...
                >
                  <TableCell className="py-4 pl-6">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-8 w-8 border border-slate-100 shadow-sm overflow-hidden text-indigo-600">
                        {reg.photoUrl ? (
                  ...
                  <TableCell className="py-4">
                    <div className="flex flex-col gap-0.5">
                      <div className="flex items-center gap-1.5 text-[11px] font-bold text-indigo-700 uppercase tracking-tight">
                        {reg.staffType === "DRIVER" ? <Truck size={12} /> : <ShieldCheck size={12} />}
                        {reg.staffType}
                      </div>
                  ...
                  <TableCell className="py-4">
                    {reg.staffType === "DRIVER" ? (
                      ...
                      <span className="text-[10px] text-slate-300 uppercase tracking-widest font-bold">N/A</span>
                    )}
                  </TableCell>
                  <TableCell className="py-4">
