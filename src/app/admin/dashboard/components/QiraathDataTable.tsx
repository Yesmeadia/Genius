...
import { QiraathRegistration } from "../types";
...
interface QiraathDataTableProps {
  data: QiraathRegistration[];
  searchTerm: string;
...
export function QiraathDataTable({
  data,
  searchTerm,
  setSearchTerm,
  itemsPerPage,
  ...
  const getSchoolName = (schoolId: string) => {
    for (const zone of locations) {
      const school = zone.schools.find(s => s.id === schoolId || s.name === schoolId);
      ...
      <CardHeader className="bg-white p-6 flex flex-col xl:flex-row justify-between items-start xl:items-center gap-4 border-b border-slate-50">
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full xl:w-auto">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button
              onClick={() => {
                import("@/lib/exportUtils").then(m => {
                  m.generateQiraathExportPDF(data, "QIRA'A Registration Registry", "qiraath_registry_data");
                });
              }}
            ...
            <Button
              variant="outline"
              onClick={() => generateBatchAccessPasses(data, 'Qiraath_Access_Passes', 'qiraath')}
              className="h-9 px-3 text-[10px] uppercase tracking-widest font-bold text-emerald-600 border-emerald-100 bg-emerald-50 hover:bg-emerald-100 transition-all rounded-xl shadow-sm"
            >
                ...
                >
                  <TableCell className="py-4 pl-6">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-8 w-8 border border-slate-100 shadow-sm overflow-hidden text-emerald-600">
                        {reg.photoUrl ? (
                  ...
                  <TableCell className="py-4">
                    <div className="flex flex-col gap-0.5">
                      ...
                      <span className="text-[9px] font-medium text-slate-500 uppercase tracking-wider line-clamp-1">
                  ...
                  <TableCell className="py-4 text-right pr-6">
                    <Link href={`/admin/dashboard/qiraath/${reg.id}`}>
                      <Button
                        variant="ghost"
                        size="sm"
                        className="h-8 px-3 text-[10px] font-bold uppercase tracking-widest text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50 rounded-xl transition-all"
                      >
                        View Profile
                      </Button>
                    </Link>
