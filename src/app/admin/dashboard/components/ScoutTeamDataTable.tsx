...
interface ScoutTeamDataTableProps {
  data: ScoutTeamRegistration[];
  searchTerm: string;
...
export function ScoutTeamDataTable({
  data,
  searchTerm,
  setSearchTerm,
  itemsPerPage,
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Button 
              onClick={() => {
                ...
                const pdfTitle = parts.length > 0 ? parts.join(" | ") : "Scout Team Registration Registry";
                const pdfFilename = isFiltered ? "scout_team_data_filtered" : "scout_team_data_master";

                import("@/lib/exportUtils").then(m => {
                  // Reusing volunteer export or similar
                  m.generateVolunteerExportPDF(data as any, pdfTitle, pdfFilename);
                });
              }}
                ...
                >
                  <TableCell className="py-4 pl-6">
                    <div className="flex items-center gap-4">
                      <Avatar className="h-8 w-8 border border-slate-100 shadow-sm overflow-hidden text-blue-600">
                        {reg.photoUrl ? (
                  ...
                  <TableCell className="py-4 text-right pr-6">
                    <Link href={`/admin/dashboard/scout-team/${reg.id}`}>
                      <Button 
                        variant="ghost" 
                        size="sm" 
                        className="h-8 px-3 text-[10px] font-bold uppercase tracking-widest text-blue-600 hover:text-blue-700 hover:bg-blue-50 rounded-xl transition-all"
                      >
                        View Profile
                      </Button>
                    </Link>
