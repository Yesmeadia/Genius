...
export default function GuestProfilePage() {
  ...
  useEffect(() => {
    ...
    const fetchGuest = async () => {
      try {
        const docRef = doc(db, "guest_registrations", id as string);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setRegistration({ id: docSnap.id, ...docSnap.data() } as GuestRegistration);
        } else {
        ...
        {editMode && (
          <div className="flex items-center gap-3 bg-amber-50 border border-amber-100 rounded-2xl px-5 py-3">
            ...
            <p className="text-[11px] font-normal text-amber-700 uppercase tracking-widest">
              Edit Mode Active — Update Guest details below
            </p>
          </div>
        )}

        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
          {/* Left Column (Cloned from Yesian profile) */}
          ...
          <div className="lg:col-span-9 space-y-8">
            <Card className="profile-card border-none shadow-xl shadow-slate-200/50 rounded-3xl bg-white overflow-hidden">
              <CardHeader className="p-8 border-b border-slate-50 bg-slate-50/30">
                ...
                <CardDescription className="text-slate-400 text-xs uppercase tracking-widest font-normal pt-1">Event guest log</CardDescription>
              </CardHeader>
              <CardContent className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                {editMode ? (
                  <>
                    ...
                    <div className="space-y-2 md:col-span-2">
                      <Label className="text-[11px] font-normal text-slate-400 uppercase tracking-[0.2em]">Permanent Address</Label>
                      <Input
                  ...
                  <>
                    ...
                    <div className="space-y-8">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <div className="text-[11px] font-normal text-slate-300 uppercase tracking-[0.2em] mb-1">Locale Registration</div>
                          <div className="text-base font-normal text-slate-800 leading-snug">{registration.address}</div>
                        </div>
                      </div>
                    </div>
                  </>
