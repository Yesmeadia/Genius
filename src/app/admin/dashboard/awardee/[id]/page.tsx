...
import { AwardeeRegistration, Accompaniment } from "../../types";
...
export default function AwardeeProfilePage() {
  const { id } = useParams();
  const router = useRouter();
  const { user, loading: authLoading } = useAuth(true);
  const [registration, setRegistration] = useState<AwardeeRegistration | null>(null);
  ...
  const [editForm, setEditForm] = useState<AwardeeEditForm | null>(null);
  ...
  useEffect(() => {
    ...
    const fetchAwardee = async () => {
      try {
        const docRef = doc(db, "awardee_registrations", id as string);
        const docSnap = await getDoc(docRef);
        if (docSnap.exists()) {
          setRegistration({ id: docSnap.id, ...docSnap.data() } as AwardeeRegistration);
        } else {
          router.push("/admin/dashboard/awardee");
        }
      } catch (error) {
  ...
  const enterEditMode = () => {
    ...
    setEditForm({
      ...
      selectionType: registration.selectionType || "",
  ...
  const updateField = (field: keyof AwardeeEditForm, value: any) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (photoFile) {
        // Use the same path as in registration form
        const storageRef = ref(storage, `photos/awardee_${Date.now()}_${photoFile.name}`);
      ...
      await updateDoc(doc(db, "awardee_registrations", id as string), updateData);
      ...
      <div ref={containerRef} className="max-w-6xl mx-auto space-y-8">
        ...
        {editMode && (
          <div className="flex items-center gap-3 bg-violet-50 border border-violet-100 rounded-2xl px-5 py-3">
            <Pencil size={14} className="text-violet-500 shrink-0" />
            <p className="text-[11px] font-normal text-violet-700 uppercase tracking-widest">
              Edit Mode Active — Update Awardee details below
            </p>
          </div>
        )}
              ...
              <CardContent className="p-8 text-center">
                {editMode ? (
                  ...
                  <>
                    ...
                    <p className="text-sm font-normal text-violet-600 uppercase tracking-[0.2em] mb-6">
                      AWARDEE
                    </p>
          ...
          <div className="lg:col-span-9 space-y-8">
            <Card className="profile-card border-none shadow-xl shadow-slate-200/50 rounded-3xl bg-white overflow-hidden">
              <CardHeader className="p-8 border-b border-slate-50 bg-slate-50/30">
                ...
                <CardDescription className="text-slate-400 text-xs uppercase tracking-widest font-normal pt-1">Awardee Information</CardDescription>
              </CardHeader>
              <CardContent className="p-8 grid grid-cols-1 md:grid-cols-2 gap-10">
                {editMode ? (
                  <>
                    ...
                    <div className="space-y-2">
                      <Label className="text-[11px] font-normal text-slate-400 uppercase tracking-[0.2em]">Selection Type</Label>
                      <Select
                        value={editForm?.selectionType || ""}
                        onValueChange={v => updateField("selectionType", v)}
                      >
                        <SelectTrigger className="border-slate-100 bg-slate-50">
                  ...
                  <>
                      ...
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <div className="text-base font-normal text-slate-800 leading-snug uppercase font-bold tracking-widest">
                            ...
                            <div className="text-[11px] text-violet-600 font-medium mt-1 uppercase tracking-tight">
                    ...
                    <div className="space-y-8">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          <div className="text-[11px] font-normal text-slate-300 uppercase tracking-[0.2em] mb-1">Rank & Selection Type</div>
                          <div className="text-base font-normal text-slate-800 leading-snug uppercase font-bold tracking-widest">
                            {registration.rank || "N/A"}
                            <div className="text-[11px] text-slate-500 font-medium mt-1 uppercase tracking-tight">
                              {registration.selectionType || "N/A"}
                            </div>
                          </div>
                        </div>
                      </div>
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-slate-400 text-xs uppercase tracking-widest font-normal pt-1">
                      {!registration.withParent && !editMode ? 'No accompaniment registered' : 'Details for event access card generation'}
                    </CardDescription>
                  </div>
                  {editMode && (
                    <div className="flex items-center gap-3">
                    ...
                    <div className="space-y-6 animate-in fade-in slide-in-from-top-2 duration-300">
                      {editForm.accompaniments.map((acc, index) => (
                        <div key={index} className="relative p-5 bg-slate-50/50 rounded-2xl border border-slate-100 space-y-4">
                          <div className="flex items-center justify-between">
                            <span className="text-[10px] font-bold uppercase tracking-widest text-violet-600/70">Person {index + 1}</span>
                      ...
                      {editForm.accompaniments.length < 3 && (
                        <Button
                          ...
                          className="w-full h-11 border-dashed border-slate-200 hover:border-violet-300 hover:bg-violet-50/30 text-slate-500 hover:text-violet-600 transition-all rounded-xl flex items-center justify-center gap-2"
                        >
                          <Plus size={16} />
                  ...
                  (registration.withParent && registration.accompaniments && registration.accompaniments.length > 0) ? (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {registration.accompaniments.map((acc, idx) => (
                        <div key={idx} className="p-6 rounded-[24px] bg-slate-50/50 border border-slate-100/50 hover:border-violet-100 transition-all group">
                    ...
                    <div className="py-3 px-4 flex items-center gap-4 bg-slate-50/50 rounded-2xl border border-dashed border-slate-200/60">
                      ...
                      <div>
                        ...
                        <p className="text-slate-400 text-[10px] uppercase tracking-tight mt-0.5">This awardee is registered as an individual</p>
                      </div>
                    </div>
                  )
                )}
