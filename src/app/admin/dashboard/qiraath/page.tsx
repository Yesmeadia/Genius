...
import { QiraathDataTable } from "../components/QiraathDataTable";

export default function QiraathPage() {
  const {
  ...
  const filterOptions = useMemo(() => {
    const zones = Array.from(new Set(qiraathRegistrations.map((r: any) => r.zone))).filter(Boolean).sort();
    let schools = [];
    if (filterZone && filterZone !== "all") {
      schools = Array.from(new Set(qiraathRegistrations.filter((r: any) => r.zone === filterZone).map((r: any) => r.school))).filter(Boolean).sort();
    } else {
      schools = Array.from(new Set(qiraathRegistrations.map((r: any) => r.school))).filter(Boolean).sort();
    }
    const classes = Array.from(new Set(qiraathRegistrations.map((r: any) => r.className))).filter(Boolean).sort();
    return { zones, schools, classes };
  }, [qiraathRegistrations, filterZone]);

  const filteredQiraath = useMemo(() => qiraathRegistrations.filter(r => {
  ...
  }), [qiraathRegistrations, searchTerm, filterZone, filterSchool, filterClass, filterGender]);

  return (
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-700">
      <QiraathDataTable
        data={filteredQiraath}
