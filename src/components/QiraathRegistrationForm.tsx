...
interface QiraathRegistrationData {
  name: string;
  gender: string;
...
export default function QiraathRegistrationForm() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<QiraathRegistrationData>({
    defaultValues: {
      name: "",
      gender: "",
      whatsappNumber: "",
  ...
  const onSubmit = async (data: QiraathRegistrationData) => {
    setIsSubmitting(true);
    try {
      let photoUrl = "";
      if (data.photo && data.photo[0]) {
        const file = data.photo[0];
        const storageRef = ref(storage, `photos/qiraath_${Date.now()}_${file.name}`);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 md:space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              ...
              <div className="space-y-2">
                ...
                <Controller
                  name="school"
                  control={control}
                  rules={{ required: "School from zone is required" }}
                  render={({ field }) => (
                    <Select
        ...
        <Card className="form-card border-none shadow-none bg-transparent">
          ...
          <CardContent className="px-0 space-y-4 md:space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-6">
              <div className="space-y-2">
                <Label className={`text-xs font-normal uppercase tracking-wider ${errors.rank ? "text-destructive" : "text-slate-500"}`}>
                  Rank
                </Label>
                <Controller
                  name="rank"
                  ...
                  render={({ field }) => (
                    <Select
                      ...
                      value={field.value}
                    >
                      <SelectTrigger className={`h-10 md:h-12 bg-white/50 focus:ring-indigo-500/20 uppercase ${errors.rank ? "border-destructive" : ""}`}>
                        <SelectValue placeholder="SELECT RANK" />
                      </SelectTrigger>
                      <SelectContent>
                        {["FIRST", "SECOND", "THIRD"].map((rank) => (
                          <SelectItem key={rank} value={rank} className="uppercase">
                            {rank}
                          </SelectItem>
                        ))}
                      </SelectContent>
        ...
        <Card className="form-card border-none shadow-none bg-transparent">
          <CardHeader className="px-0">
            <CardTitle className="text-xl font-normal text-slate-900 tracking-tight">Participant Photo</CardTitle>
          </CardHeader>
