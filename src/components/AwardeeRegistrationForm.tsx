...
export default function AwardeeRegistrationForm() {
  ...
  const {
    ...
    setValue,
    formState: { errors },
  } = useForm<AwardeeRegistrationData>({
    defaultValues: {
      name: "",
      gender: "",
      whatsappNumber: "",
  ...
  const onSubmit = async (data: AwardeeRegistrationData) => {
    setIsSubmitting(true);
    try {
      let photoUrl = "";
      if (data.photo && data.photo[0]) {
        const file = data.photo[0];
        const storageRef = ref(storage, `photos/awardee_${Date.now()}_${file.name}`);
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 md:space-y-8">
        ...
        <Card className="form-card border-none shadow-none bg-transparent">
          <CardHeader className="px-0">
            <CardTitle className="text-xl font-normal text-slate-900 tracking-tight">Awardee Photo</CardTitle>
          </CardHeader>
          <CardContent className="px-0">
            <div className="space-y-3">
